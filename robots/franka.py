...
class Franka(Robot):
    ...
    def __init__(
        ...
        self.camera = Camera(
            ...
            translation=np.array([-0.15, -0, -0.05]),
            frequency=20,
            resolution=(256, 256),
            # orientation=rot_utils.euler_angles_to_quats(np.array([180, -23, 90]), degrees=True),
            orientation=np.array([-0.16957, -0.69206, 0.68009, 0.17255])
        )
        self.camera.set_local_pose(orientation=np.array([0.22437, 0.67057, -0.67057, -0.22437]), camera_axes="usd")
        self.camera.set_clipping_range(near_distance=0.1)
