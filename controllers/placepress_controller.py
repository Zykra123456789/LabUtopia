...
class PlacePressTaskController(BaseController):
    ...
    def _check_phase_success(self):
        ...
        if self.current_phase == Phase.PICKING:
            ...
            print(f"object_pos is not None: {object_pos is not None}")  # True
            print(f"object_pos[2] <= 0.86: {object_pos[2] <= 0.86}")  # True (0.85 <= 0.86)
            print(f"abs(object_pos[0] - target_position[0]) < 0.05: {abs(object_pos[0] - target_position[0]) < 0.05}")  # True (abs(1.0 - 1.01) = 0.01 < 0.05)
            print(f"abs(object_pos[1] - target_position[1]) < 0.05: {abs(object_pos[1] - target_position[1]) < 0.05}")  # True (abs(2.0 - 2.02) = 0.02 < 0.05)

            # Initial success value
            print(f"Base success: {success}")  # True (all conditions met)
    ...
    def _step_collect(self, state):
        ...
        if success:
            print("activate controller",self.active_controller)
            if self.current_phase == Phase.PICKING:
