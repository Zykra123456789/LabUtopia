...
class OpenController(BaseController):
    """
    Controller for opening drawers/doors.
    
    State machine controller for opening drawers or cabinet doors.

    States include:
    - State 0: Approach handle
    - State 1: Further approach
    - State 2: Close gripper
    - State 3: Open drawer/door to target position (using SLERP for doors)
    - State 4: Open gripper
    - State 5: Move back (for drawers)

    Args:
        name (str): Controller identifier
        events_dt (list of float, optional): Duration for each state. Defaults to [0.01, 0.01, ...]
        speed (float, optional): Action speed multiplier. Defaults to 1.0
        door_width (float, optional): Door width (rotation radius). Defaults to 0.3
        door_open_direction (str, optional): Door opening direction, "clockwise" or "counterclockwise". Defaults to "counterclockwise"
    """
    ...
    def forward(
        ...
        revolute_joint_position: np.ndarray,
    ...
    def _execute_phase(self, handle_position, end_effector_orientation, current_joint_positions, revolute_joint_position, gripper_position):
        """Execute current phase of grasping action"""
        if self.furniture_type == "drawer":
            return self._execute_drawer_phase(handle_position, end_effector_orientation, current_joint_positions)
        else:
            return self._execute_door_phase(handle_position, end_effector_orientation, current_joint_positions, revolute_joint_position, gripper_position)
    
    def _execute_drawer_phase(self, handle_position, end_effector_orientation, current_joint_positions):
        ...
        if self._event == 0:
            ...
            gripper_distance = 0.02 / get_stage_units()
            ...
            handle_position[0] -= 0.02 / get_stage_units()
            target_joint_positions = self._cspace_controller.forward(
                target_end_effector_position=handle_position, 
                target_end_effector_orientation=end_effector_orientation
            ) 
    ...
    def _execute_door_phase(self, handle_position, end_effector_orientation, current_joint_positions, revolute_joint_position, gripper_position):
        ...
        if self._event == 0:
            ...
            if self.position_rotation_interp_iter is None:
                ...
                num_interpolation = int(1000 * np.linalg.norm(self.start_position - self.target_position))
