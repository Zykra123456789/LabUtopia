...
class ObjectUtils:
    ...
    def get_object_position(self, object_name: str = None, object_path: str = None) -> np.ndarray:
        """Get the world-space center position of an object."""
    ...
    def get_pick_position(self, object_name: str = None, object_path: str = None) -> np.ndarray:
        ...
        position = self.get_object_position(object_name, object_path)
        if position is None:
            return None
    ...
    def get_transform_position(self, object_path: str) -> np.ndarray:
        """Get the world-space position from the object's transform."""
        prim = self._stage.GetPrimAtPath(object_path)
        if not prim.IsValid():
            print(f"Object at path {object_path} not found.")
    ...
    def get_transform_quat(self, object_path: str) -> np.ndarray:
        ...
        if rotation is None:
            ...
            return np.array([rotation[1], rotation[2], rotation[3], rotation[0]])
